...
exports.update = async (req, res) => { try { const doc = await Metal.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true }); if (!doc) return res.status(404).json({ error: "Not found" }); res.json({ data: doc }); } catch (e) { res.status(500).json({ error: e.message }); } };
